...
struct DynamicLight
{
    ...
    #include "Packages/de.alpacait.dynamiclighting/AlpacaIT.DynamicLighting/Shaders/Generators/TrilinearFilter.hlsl"
...
float sample_distance_cube(uint cubeDataOffset, float3 dir)
{
    ...
    uint index = (uint)dynamic_lights_distance_cubes_index_lookup.SampleLevel(sampler_dynamic_lights_distance_cubes_index_lookup, dir, 0).r;
...
// next we prepare macro statements that shaders use to implement their fragment functions.
    ...
    \
    float4 frag (v2f i, uint triangle_index:SV_PrimitiveID, bool is_front_face:SV_IsFrontFace) : SV_Target
    ...
    float4 frag (v2f i) : SV_Target\
    {\
        return SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.uv0);\
    }
#endif
#endif
